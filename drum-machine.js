...
var data = {
  ...
  tracks: [createTrack("gold", note(audio, 880)),
           ...
           createTrack("pink", note(audio, 452)),
...
function travelSpeed(){
    ...
    sendButton.onclick = function(){
      var keyBox = document.getElementById("speed");
      b = keyBox.value;
      return b;
    }
...
finishRecording.onclick = function finishRecording(){
  ...
  for(var i=0; i<=sounds.length;i++){
    data.tracks.forEach(function(track, row) {
        track.steps.forEach(function(on, column) {
            if (row==sounds[i].widths){
            track.steps[sounds[i].heights] = !on;
            }
        });
    });
  }
}
deleteRecording.onclick = function deleteRecording(){
